                ...
                for (int i = 0; i < krArray.Count(); i++)
                {
                    foreach (Keyframe kf in krArray[i].Value.OrderedGenericList)
                    {
                        if (_tl.UncommittedRackChanges[krArray[i].Key] != null && _tl.UncommittedRackChanges[krArray[i].Key].Keyframes.Contains(kf))
                        {
                            ...
                            g.DrawImage(Resources.moveKeyframes, (int)((kf.Position + delta) * frameSizeInPixels) - 12 + 8, 35 + (i * 24));
                            continue;
                        }

                        g.DrawImage(Keyframe.KeyframeInterp[kf.InterpolationType], (int)(kf.Position * frameSizeInPixels) - 12, 0 + 35 + (i * 24));
                        if (_tl.SelectedKeyframes.Contains(kf))
                        {
                            g.DrawImage(Resources.fSelected, (int)(kf.Position * frameSizeInPixels) - 12 + 8, 35 + (i * 24) + 8);
                        }
                    }
                }
